  ...
  return (
    <>
      <Box sx={{flexDirection: {xs: 'column', md: 'row'}}} display={'flex'} alignItems={'center'} gap={2} justifyContent={'space-evenly'} component={'main'} minHeight="85dvh">
        ...
        <article>
          {isAboutPage && (
            <Typography variant={isSmallScreen ? 'h5' : 'h4'} paddingY={3} component={'h1'} color={'primary'} paddingLeft={1}>
              {businessInfo.title}
            </Typography>
          )}
          <Paper elevation={3} sx={{padding: 2}}>
            <Typography component={'h3'} className="title" color={'primary'} sx={{fontSize: '20px'}}>
              {isAboutPage ? businessInfo.subtitle : 'About Us:'}{' '}
            </Typography>
        ...
        {loading ? (
          ...
          <Card sx={{width: isMediumScreen ? 400 : 'none'}}>
            <CardMedia component="img" height="350" image={defaultImage(businessInfo.imageUrl)} alt={defaultImage(businessInfo.imageAlt)} />
          </Card>
        ) : (
