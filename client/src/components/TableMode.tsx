...
export default function TableMode({cards}: TableModeProps) {
  ...
  const excludedProperties = ['imageUrl', 'imageAlt', '_id', 'description', 'houseNumber', 'web', 'state', 'zip', 'user_id', '__v', 'isFavorite', 'rating', 'liked'];
          ...
          <TableBody>
            {cards.map((row, index) => (
              <StyledTableRow key={index}>
                ...
                {Object.values(row).map((value, index) => !excludedProperties.includes(Object.keys(row)[index]) && <StyledTableCell key={index}>{value}</StyledTableCell>)}
              </StyledTableRow>
            ))}
          </TableBody>
        </Table>
